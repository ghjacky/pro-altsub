...
func Subscribe(ctx *gin.Context) {
	...
	if rcvId == 0 {
		ctx.JSON(http.StatusOK, newHttpResponse(&ErrorBadRequest, "Subscribe()", nil, nil))
		return
	}
	...
	if err := subscribe.Subscribe(rcv, rs); err != nil {
		ctx.JSON(http.StatusOK, newHttpResponse(&ErrorFailedToSubscribeRules, "Subscribe()", nil, nil))
		return
	}
	ctx.JSON(http.StatusOK, newHttpResponse(nil, "Subscribe()", nil, nil))
}

func Assign(ctx *gin.Context) {
	rId, _ := strconv.Atoi(ctx.Param("id"))
	if rId == 0 {
		ctx.JSON(http.StatusOK, newHttpResponse(&ErrorBadRequest, "Assign()", nil, nil))
		return
	}
	var sub = models.MSubscribe{}
	...
	if err := subscribe.Assign(r, rcvs); err != nil {
		ctx.JSON(http.StatusOK, newHttpResponse(&ErrorFailedToAssignRules, "Assign()", nil, nil))
		return
	}
	ctx.JSON(http.StatusOK, newHttpResponse(nil, "Assign()", nil, nil))
}

func FetchSubscribe(ctx *gin.Context) {
	var pq = models.PageQuery{}
	if err := ctx.BindQuery(&pq); err != nil {
		ctx.JSON(http.StatusOK, newHttpResponse(&ErrorBadRequest, "FetchSubscribe()", nil, nil))
		return
	}
	var ss = models.MSubscribes{TX: base.DB().Begin(), PQ: pq}
	if err := subscribe.Fetch(&ss); err != nil {
		ss.TX.Rollback()
		ctx.JSON(http.StatusOK, newHttpResponse(&ErrorFailedToFetchSubscribes, "FetchSubscribe()", nil, nil))
		return
	}
	ss.TX.Commit()
	ctx.JSON(http.StatusOK, newHttpResponse(nil, "FetchSubscribe()", ss.All, map[string]interface{}{"total": ss.PQ.Total}))
}
